...
namespace OW {
    int abletotread = 0;
    int entitytime = 0;
	void entity_scan_thread(const offset::ScanOffsets& offsets);
	void void entity_thread();
}

void OW::entity_scan_thread(const offset::ScanOffsets& offsets) {
    while (Config::doingentity == 1) {
        if (abletotread == 0) {
            // 1. 반환(Return) 대신 내부에서 직접 로컬 벡터 생성 (메모리 재할당 방지를 위해 reserve 사용)
            std::vector<std::pair<uint64_t, uint64_t>> local_result;
            local_result.reserve(256); // 예상되는 최대 엔티티 수만큼 미리 예약 (성능 향상)
            ...
            // 2. new/delete 대신 std::vector 사용 (메모리 릭 원천 차단 및 예외 안전성 확보)
            std::vector<Entity> raw_list(entity_list_size);

            if (ReadProcessMemory(SDK->hProcess, (LPCVOID)entity_list, raw_list.data(), mbi.RegionSize, nullptr)) {
                for (size_t i = 0; i < entity_list_size; ++i) {
                    ...
                    uint32_t unique_id = SDK->RPM<uint32_t>(common_linker + offsets.Link_UniqueId);
                    
                    for (size_t x = 0; x < entity_list_size; ++x) {
                        uint64_t possible_common = raw_list[x].entity;
                        if (!possible_common) continue;

                        if (SDK->RPM<uint32_t>(possible_common + offsets.Common_UniqueId) == unique_id) {
                            local_result.emplace_back(possible_common, cur_entity);
                            break;
                        } 
                        else {
                            uint64_t Ptr = SDK->RPM<uint64_t>(possible_common + offsets.Component_Ptr) & 0xFFFFFFFFFFFFFFC0;
                            if (Ptr < 0xFFFFFFFFFFFFFFEF) {
                                uint64_t EntityID = SDK->RPM<uint64_t>(Ptr + offsets.Entity_Id);
            ...
            // 3. 복사 오버헤드 없이 전역 변수로 소유권 이전 (가장 핵심적인 최적화)
            ow_entities_scan = std::move(local_result);
            abletotread = 1;
        }
        Sleep(10);
    }
